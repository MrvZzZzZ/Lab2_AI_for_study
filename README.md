...
| --- | --- | --- | --- | --- | --- |
| 8 | Нет | Kaggle Fake News Dataset | Предобученные | Векторы -> Umap -> Кластеризация (Kmeans, HDBSCAN) + BertTopic | 425 Лёдов, Миняев, Роева, Перьков, Пономарев  |
...
При кластеризации с использованием HDBSCAN проведена оценка по метрике DBCV (Density-Based Clustering Validation), показавшая результат -0.24 в диапазоне от -1 до 1, что свидетельствует о пересечении и неустойчивости кластеров.
...
На рисунках 2 и 3 произведено разделение на фейковые и реальные новости.

![<img src="/graph1.png">](https://github.com/MrvZzZzZ/Lab2_AI_for_study/blob/main/graph1.png)

								Рисунок 2 - Проекция кластеров с фейковыми новостями в двумерной плоскости

![<img src="/graph2.png">](https://github.com/MrvZzZzZ/Lab2_AI_for_study/blob/main/graph2.png)

								Рисунок 3 - Проекция кластеров с реальными новостямии в двумерной плоскости
На рисунке 4 для кластеров с номерами от -1 до 30 выведено нормированное соотношение новостных статей в этих кластерах.

![<img src="/graph3.png">](https://github.com/MrvZzZzZ/Lab2_AI_for_study/blob/main/graph3.png)

										Рисунок 4 - Соотношение новотных статей в кластере

Для полученных данных по границе 0.9 для нормированных значений соотношения статей в кластере определим принадлежность кластеров к группам "фейк" и "реальные"

| Тип кластера | Процент |
| --- | --- |
| real | 28.351% |
| fake | 20.619% | 
| mix | 51.031% |
...
Для интерпретации полученных результатов проведён анализ получаемых кластеров для выделения какие из них относятся к фейкам, что также вносит ясность, насколько качественно модель кластеризирует новости по категориям "фейковые" и "реальные".
